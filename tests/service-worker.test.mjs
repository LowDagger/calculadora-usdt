...
function createWorker({ cachedResponse, networkResponse } = {}) {
  ...
  const context = {
    ...
    caches: {
      open: async () => cache,
      match: async () => cachedResponse,
      keys: async () => ['calcuflow-v52', 'calcuflow-v53', 'calcuflow-v54'],
...
test('non-precache static assets remain stale-while-revalidate', async () => {
  const cached = { source: 'cache' };
  const network = { clone: () => ({ source: 'clone' }), status: 200, source: 'network' };
  const worker = createWorker({ cachedResponse: cached, networkResponse: network });

  assert.equal(await dispatchFetch(worker, request('/uploads/custom-bank.png')), cached);
...
test('activation deletes old versioned caches and retains the current cache', async () => {
  ...
  assert.deepEqual(worker.deletedCaches, ['calcuflow-v52', 'calcuflow-v53']);
});
