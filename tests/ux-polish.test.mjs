...
test('marks result-card symbols as decorative and bumps the PWA cache', () => {
  ...
  assert.match(serviceWorker, /const APP_VERSION\s+= '48';/);
...
test('keeps Community actions balanced with the unread marker and compact footer', () => {
  ...
  assert.match(html, /id="toggleQrBtn"[\s\S]*?>Mostrar QR</);
  assert.match(html, /id="supportQrBox"[\s\S]*?style="display: none;"/);
  assert.match(html, /<img[^>]*src="assets\/telegram-qr\.jpg"[^>]*alt="Código QR del grupo de Telegram de CalcuFlow"[^>]*\/?>/);
  assert.match(serviceWorker, /'\/assets\/telegram-qr\.jpg'/);
