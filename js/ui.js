...
export function renderResult(r) {
  ...
  const bankFeeLabel = r.bankFeeSteps?.length
    ? r.bankFeeSteps.map(fee => `${money(fee, 1)}%`).join(' + ')
    : `${money(r.cardPct, 1)}%`;
  els.flowCard.innerHTML    = '-' + money(r.cardFeeUsd, 2) + ' <span class="value-unit">USD</span> (' + bankFeeLabel + ')';
  ...
  els.formulaText.innerHTML = `
    ...
    <strong>Monto máximo BPay:</strong> ${money(r.safeGateway.allowedBankSpend, 2)} ÷ ${r.bankFeeSteps?.length ? r.bankFeeSteps.map(fee => `(1 + ${money(fee, 1)}%)`).join(' ÷ ') : (1 + r.cardPct / 100).toFixed(4)}, truncado a centavos = ${money(r.afterCard, 2)} USD.<br>
